...
- 状态：`stale`（`prod.md` 1.20 已批准四角色 URL 路由重构，尚未完成实现与验证；下列 1.19 证据保留为历史基线）
- 检查日期：2026-08-12
...
- 待验证 `prod.md` 6.4 的四角色规范路径、父路径替换、静态页与对象深链、角色不匹配、安全 404、查询参数规范化和浏览器前进/后退。
- 待验证未提交表单离开保护、提交前流程刷新回退、页面标题/主标题/`aria-current` 同步，以及现有 `1440 × 1024`、`1024 × 768`、顾客 `360 × 800` 视觉与交互没有回归。
- 待验证真实链接的复制、新标签和键盘语义，页面/Tab/详情焦点与滚动恢复，硬刷新/关闭的原生未提交保护，以及目标 URL 加载期间旧页面、旧角色或旧对象数据不会闪现。
- 待验证生产 Next.js 嵌套路由与共享布局、URL 单一真相、四角色业务路径 `noindex, nofollow`，以及正式原型集中式 History 路由、Sites 深链刷新和原型专用 QA 参数兼容。
- 待验证全部规范路径清单、每个一级页面和稳定 Tab，以及代表性详情、刷新、前进/后退、筛选、角色不匹配、无会话深链、无效路径和未提交保护。恢复 `current` 前必须重新通过管理端 `1440 × 1024`、`1024 × 768` 和顾客关键路径 `360 × 800` 的控制台、焦点、横向溢出及旧数据闪现检查。
- 待验证同角色导航期间共享角色布局、上下文、实时失效连接、主演示和沙箱工具持续挂载，上一业务工作区不会作为隐藏实例继续请求；交付不得同时保留旧根页内存导航和新规范路由。
...
- 根因是共享壳将工作区限制为固定可视高度并裁切溢出，而直接挂载的业务 `<main>` 只有最小高度，超出内容没有滚动宿主。现由非顾客角色的直接 `<main>` 占满工作区并承担受控纵向滚动；保留带检查器工作台自己的列级滚动。
- 应用内浏览器在精确 `1024 × 768 CSS px` 逐项复核店员报修、本人班次、交接快照、店长经营看板、员工与排班、审计详情、总部门店比较、总部人员汇总和总部审计。所有受影响主区均为工作区同高且 `overflow-y: auto`；点击列表或表格末项会实际移动对应主区滚动位置，不再被壳层裁切。页面级无横向溢出。
- `WEB-C04` 桌面栏改为 H5 画布内的粘性末行：在批注同档 `1327 × 866` 中，其左右边缘与 `.customer-h5` 相差不超过 `1px`，底边贴齐 `.role-workspace`；短商品目录同样以弹性页面结构贴底，长目录保持在画布内粘附。`1440 × 1024` 复核结果相同。
- `360 × 800 CSS px` 下保留移动端固定操作栏和 `96px` 内容安全留白；栏宽等于视口宽度，`documentElement.scrollWidth <= clientWidth`，不遮挡末项。
- 新增 Chromium 回归覆盖 `direct role pages scroll inside the shared workspace instead of being clipped` 与 `WEB-C04 keeps the desktop cart action within the H5 canvas and preserves its mobile fixed layout`；两条均通过。应用内浏览器在上述路径未出现页面错误状态或开发错误覆盖层。
...
- Standards 与 Spec 多轮固定点复审全部清零。完整 `JINGSHU_E2E_WEB_ORIGIN=http://127.0.0.1:3017 pnpm verify` 通过：332 个 Vitest 单元测试、2 个开发启动测试、177 个真实 PostgreSQL/API 集成测试、2 个设计内容基线测试与 58 个 Chromium E2E；格式、lint、strict TypeScript、生产构建、工作区边界和敏感数据扫描全部通过，high 级安全审计门禁通过（仅 1 个 moderate）。正式管理端原型生产构建与 4 个 Sites 包装测试通过。
...
- 导出确认覆盖预约、订单、库存流水、报修、排班和审计六种数据类型；审计导出复用页面当前结果筛选与排序，其他类型使用各自搜索、精确状态和合法排序控件。六类预览均逐列、逐行呈现当前所见筛选结果，并与下载共用同一格式化结果。弹窗持续展示固定棱镜旗舰店、经营日范围和预计行数，初始焦点、Tab 循环、Escape 与触发器焦点返回均通过。
- `design/ticket-23-export-ready-1024x768.png`、`ticket-23-export-failure-1024x768.png` 和 `ticket-23-export-success-1024x768.png` 分别记录报修当前结果预览、受控网络失败与原筛选重试成功。失败时没有声称已下载，原类型/筛选/范围均保留；恢复网络后相同请求成功返回文件名、行数并追加导出审计。故障注入后已恢复浏览器网络规则。
...
- 第 2 轮 P1 已关闭：代码审查指出五类业务导出只提供独立条件控件、没有把“当前所见”结果呈现在确认页，同时非法数据类型/排序组合会被静默降级。最终预览返回全部格式化列与行，确认页直接呈现当前结果；合约改为按数据类型区分的联合类型，API 对每类状态和排序字段做精确校验，非法组合返回 `422`。同轮把“服务端记录时间”统一为契约词汇“服务器记录时间”，覆盖文档只保留页面、状态和旅程。真实浏览器在 `1024 × 768` 下复核报修 2 行预览、无金额排序、状态枚举、弹窗完整可见且页面无横向溢出；新标签 warning/error 为 0。当前没有仍需处理的 P0、P1 或 P2 差异。
- 完整 `pnpm verify` 通过：331 个 Vitest 单元测试、2 个开发启动测试、173 个真实 PostgreSQL/API 集成测试、2 个设计内容基线测试与 56 个 Chromium E2E；全部格式、lint、strict TypeScript、构建、工作区边界与敏感数据扫描通过，high 级依赖审计仅报告 1 个不阻塞门禁的 moderate 漏洞。正式管理端原型生产构建与 4 个 Sites 包装测试通过。
...
## Ticket 26 — 总部三店比较、全沙箱审计与导出

### 对照源与证据

- 选定视觉方向继续使用 `design/reference/selected-night-operations-console.png`；同状态页面源使用正式原型 `design/implementation-ticket02-chain-dashboard-1440.png` 与 `design/implementation-ticket02-store-compare-1024.png`。
- `1440 × 1024` 连锁看板同屏对照：`design/comparison-ticket26-chain-dashboard-2880x1024.png`；生产原图为 `design/implementation-ticket26-chain-dashboard-1440x1024.png`。
- `1024 × 768` 门店比较同屏对照：`design/comparison-ticket26-store-compare-2048x768.png`；生产原图为 `design/implementation-ticket26-store-compare-1024x768.png`。

### Findings 与修复

- 首轮 P1：总部审计导出在新增链路门店列后仍保留单店审计自带的“门店”列，预览出现重复表头并触发 React 重复键错误。修复后总部审计只保留一列“门店”，预约等其他导出结构不变；新增集成断言保证所有预览列唯一，重新打开页面并完成下载后 warning/error 为 0。
- 首轮 P2：极点新店误用了琥珀色圆点作为身份标记，既偏离正式原型紫色竖条，也会与琥珀异常语义混用。修复后三店恢复青柠、青色、紫色竖条；琥珀/红色只用于异常数量和严重度。
- 代码审查 P1：库存告警曾把未生成的历史快照绘制为 `0`。修复后领域响应使用 `null` 表达缺失历史，图表断线且等价数据表显示“未生成历史快照”；摘要明确是数据截止时点的当前库存快照。
- 代码审查 P1：全沙箱审计曾只逐店读取，遗漏 `store_id = null` 的总部目录等连锁级事件；审计 CSV 同样缺失。修复后一次数据库查询覆盖三店与连锁范围，单店筛选仍严格隔离，列表、详情和 CSV 均以“连锁范围 · chain”明确展示。
- 代码审查 P1：三店下载曾分别提交导出审计，第三店失败时可能留下部分允许事件。修复后先只读准备全部数据，再以单一事务写入所选门店导出审计；无效门店回归证明失败时审计数保持为零。
- 代码审查 P1：经营异常合计曾固定下钻到报修，且只读面板缺少键盘焦点管理。修复后先展示库存、报修、考勤和交接四类构成，再进入对应事实；E2E 覆盖初始焦点、`Escape` 关闭和焦点返回，面板实现 `Tab` 循环。
- 复核 P1/P2：历史范围中的库存摘要曾沿用历史范围下钻而得到空结果；跨 `06:00` 后滚动范围曾从旧响应计算。修复后库存下钻由 API 明确返回当前经营日快照，集成测试断言行数与摘要一致；最近 7/14 日自动刷新先读取新的可用经营日集合再发起范围请求。
- 浏览器批注：总部“门店比较”的 Recharts 默认白底提示层与全局浅色标题叠加，使“经营日 08/09”缺少对比。现复用夜间运营控制台的深色提示层，并将总部标题提升为 `12px`、数据项提升为 `11px`；应用内浏览器在 `1327 × 866`、`1440 × 1024`、`1024 × 768` 下确认经营日和三店“名称 · 代码 · 数值”完整可见，提示层为 `#07141d`、标题为 `#f3fbfd`，没有页面级横向溢出，console warning/error 均为 `0`。
- 最终没有仍需处理的 P0、P1 或 P2 差异。生产页用真实种子事实，因此两家门店在当前经营日可能为零；没有用正式原型的演示数值伪造趋势。门店名称、代码、竖条和图例共同区分三店，不以颜色作为唯一编码。

### 必查表面与交互

- `1440 × 1024` 下连锁摘要、三店经营脉冲、趋势与指标摘要保持正式夜间运营控制台的炭灰/海军蓝表面、青柠主强调、青色数据和紧凑表格密度；`bodyScrollWidth = bodyClientWidth = 1440px`。
- `1024 × 768` 下导航按既有规则折叠，指标/门店筛选、口径说明、趋势、指标摘要和等价数据表完整可见；`bodyScrollWidth = bodyClientWidth = 1024px`，无裁切或页面级横向溢出。
...
- [x] ticket 26 的 `1440 × 1024` 连锁看板和 `1024 × 768` 门店比较同屏对照、八指标筛选、只读下钻、全沙箱审计与导出完成态通过，P0/P1/P2 清零。
