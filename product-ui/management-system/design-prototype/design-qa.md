...
- 正式原型源截图：`design/source-ticket03-public-entry-1440.png`、`design/source-ticket03-public-entry-1024.png`、`design/source-ticket03-public-entry-360.png`。
- 生产实现截图：`design/implementation-ticket03-public-entry-1440.png`、`design/implementation-ticket03-public-entry-1024.png`、`design/implementation-ticket03-public-entry-360.png`。
- 视口：Codex 应用内浏览器 `1440 × 1024 CSS px`、`1024 × 768 CSS px`、`360 × 800 CSS px`。
...
- 紧凑桌面合并对比：`design/qa-ticket03-public-entry-comparison-1024.png`，左侧为正式原型，右侧为生产实现。
- 移动端合并对比：`design/qa-ticket03-public-entry-comparison-360.png`，左侧为正式原型，右侧为生产实现。

## Findings

- 没有仍需处理的 P0、P1 或 P2 差异。
- 客观检查：`1440px` 桌面两侧的标题内容宽度均为 `714px`，主故事卡均为 `476 × 240px`；`1024px` 两侧标题均为 `760 × 111px`、主故事卡均为 `881 × 230px`，下一分区起点只相差 `1px`；移动端两侧内容宽度均为 `345px`，无横向溢出，标题、主故事卡与下一分区边界落点一致。
...
- 响应式与无障碍：角色入口使用语义按钮，状态页使用 live region；弹窗打开后焦点进入关闭按钮，`Tab`/`Shift+Tab` 留在弹层内，`Escape` 关闭后焦点回到“只读了解”，并保留 reduced-motion 降级。

## 浏览器与交互验证

- 应用内浏览器控制台 `warning/error`：0。
- 只读了解：打开说明弹窗后仍停留公开地址，页面本身和只读动作不发起创建请求；真实键盘检查覆盖初始焦点、反向循环、`Escape` 关闭与触发器焦点恢复。
- 真实创建：选择顾客后先建立签名、HttpOnly 的匿名访客上下文，再通过同源 Web 路由调用 Hono 与真实 Postgres，返回栖光市固定三店、`Schema 2`、`Seed 2026-08-09.1`；地址仍为 `/`，查询与片段中无沙箱标识、会话凭证或可写能力。
...
- 构建：生产 Web `pnpm --filter @jingshu/web build` 通过，并由 `next start` 的优化构建采集实现证据（无开发指示器）；正式原型 `npm run build` 通过；Sites 包装测试 `npm run test:sites` 4/4 通过。
...
- 修复后使用本文件列出的四组同状态合并证据复核，P0/P1/P2 全部清零。
...
- [x] `1440 × 1024`、`1024 × 768`、`360 × 800` 同视口全视图及重点区域合并对比完成。
